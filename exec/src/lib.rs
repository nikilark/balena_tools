...
impl ExecCommand {
    fn create_ssh_command(&self, device_uuid: &String, service: &Option<String>, command: &String) -> String {
...
impl BalenaCommand for ExecCommand {
    fn execute(&self, args : Vec<String>) {
        ...
        for device in input_devices {
            match get_device_by_name(device.as_str(), &all_devices, false) {
                Some(d) => {
                    let command = self.create_ssh_command(&d.uuid, &args.service, &args.command);
