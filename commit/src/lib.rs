...
struct Args {
    ...
    // Not equal
    #[arg(long="ne", help="Set this flat to check if commit not equal instead")]
    ne: bool,
...
pub fn execute_command(args : Vec<String>) {
    ...
    for device in input_devices {
        match get_device_by_name(device.as_str(), &all_devices) {
            Some(d) => {
                match get_device_long_info(d) {
                    Some(info) => {
                        let passed = |c : &String| if args.ne {*c != args.commit} else {*c == args.commit};
